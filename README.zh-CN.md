...
Codex Migrate 可以直接读取旧设备复制出来的 `.codex/`，或者由本工具导出的精简 `Codex/` 文件夹。程序以 rollout JSONL 为会话事实来源，将用户选中的会话安全合并到已有的本机 Codex 环境。
...
不会导出认证、Skills、配置、插件、日志、缓存、设备状态和源数据库。

> [!WARNING]
> rollout 和 HTML 可能包含敏感对话、命令输出、图片及本机路径。分享前请自行检查和脱敏。
...
2. 选择旧设备 `.codex/`、精简 `Codex/`，或只包含其中一个目录的父目录。
3. 选择需要导入的项目与会话。
